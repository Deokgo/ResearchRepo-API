...
def create_sdg_plot(selected_programs, selected_status, selected_years, sdg_dropdown_value):
    ...
    df.rename(columns={'research_count': 'Count', 'sdg': 'sdg', 'school_year': 'school_year', 'program_id': 'Program'}, inplace=True)  # Changed 'college_id' to 'program_id'
...
def create_sdg_pie_chart(selected_programs, selected_status, selected_years, sdg_dropdown_value, college):
    ...
    if sdg_dropdown_value == "ALL":
        ...
        fig = px.pie(
            ...
            values='Percentage',
            title='Percentage of Research Outputs by Program',
...
def create_sdg_research_chart(selected_programs, selected_status, selected_years, sdg_dropdown_value):
    """
...
def create_geographical_heatmap(selected_programs, selected_status, selected_years, sdg_dropdown_value):
    """
    ...
    # Check if DataFrame is empty and return None if true
    if df.empty:
...
def create_geographical_treemap(selected_programs, selected_status, selected_years, sdg_dropdown_value):
    """
...
def create_conference_participation_bar_chart(selected_programs, selected_status, selected_years, sdg_dropdown_value):
    """
...
def create_local_vs_foreign_donut_chart(selected_programs, selected_status, selected_years, sdg_dropdown_value):
    """
    ...
    df = get_local_vs_foreign_participation(
        start_year=min(selected_years),
        end_year=max(selected_years),
        country_name="Philippines",  # Assuming Philippines as 'Local'
        sdg_filter=None if sdg_dropdown_value == "ALL" else [sdg_dropdown_value],
        status_filter=selected_status,
        program_filter=selected_programs  # Changed from college_filter to program_filter
    )
...
def get_word_cloud(selected_programs, selected_status, selected_years, sdg_dropdown_value):
    """
    ...
    return fig
def generate_research_area_visualization(selected_programs, selected_status, selected_years, sdg_dropdown_value="ALL"):
    """
    ...
    # Check if 'year' column exists before proceeding with grouping
    if "year" not in df.columns and sdg_dropdown_value != "ALL":
        print("Year column is missing, cannot group by year.")
        return None
